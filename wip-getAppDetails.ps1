...
$tenantId = "2phn2l.onmicrosoft.com" # "2phn2l.onmicrosoft.com"
...
# Email the result to the specified email address
$EmailFrom = "joe@acme.com"
$EmailTo = "tstbmcll@acme.com"
$Subject = "Graph App Inventory"
$Body = "Attached is the Graph App Inventory"
$SMTPServer = "smtp.office365.com"
$SMTPPort = "587"
$Username = "NTID"
$Password = "********"
$attachment = "$((Get-Date).ToString('yyyy-MM-dd_HH-mm-ss'))_GraphAppInventory.csv"
$filename = "$((Get-Date).ToString('yyyy-MM-dd_HH-mm-ss'))_GraphAppInventory.csv"

Send-MailMessage -From $EmailFrom -To $EmailTo -Subject $Subject -Body $Body -SmtpServer $SMTPServer -port $SMTPPort -UseSsl -Credential (New-Object System.Management.Automation.PSCredential $Username, $(ConvertTo-SecureString $Password -AsPlainText -Force)) -Attachments $attachment -DeliveryNotificationOption OnFailure


#Cleanup
Remove-Variable -Name SPPerm,SPusers,OAuthperm,SPs,SP,appRoleAssignments,oauth2PermissionGrants,assignedto,resID,userId,token,authHeader,Scopes,Scope,body,uri,res,SPperm,output,objPermissions,OAuthperm,SPusers,OAuthperm,SPs,SP,appRoleAssignments,oauth2PermissionGrants,assignedto,resID,userId,token,authHeader,Scopes,Scope,body,uri,res,SPperm,output,objPermissions,OAuthperm,SPusers -ErrorAction SilentlyContinue
```
